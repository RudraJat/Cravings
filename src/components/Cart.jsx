  ...
  return (
    <div className="pt-28 min-h-screen px-6 md:px-16 pb-16 bg-gradient-to-br from-white via-green-50 to-green-100">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-extrabold text-center text-green-700 mb-8">🛒 Your Cart</h1>

        {cartItems.length === 0 ? (
          <div className="text-center text-gray-600 text-lg font-medium mt-20">
            🛍️ Your cart is empty. <Link to="/" className="text-green-600 hover:underline font-semibold">Browse restaurants</Link>
          </div>
        ) : (
          <div className="bg-white shadow-2xl rounded-3xl overflow-hidden border border-green-100">
            ...
            {cartItems.map((item, index) => {
              const itemInfo = item.card?.info || item.info || item;
              const price = itemInfo?.price ?? itemInfo?.defaultPrice ?? 0;
              const imageId = itemInfo?.imageId || itemInfo?.cloudinaryImageId || "";
              const quantity = item.quantity ?? 1;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-6 w-full md:w-2/3">
                    ...
                    />
                    <div>
                      <h2 className="text-xl font-bold text-gray-800 mb-1">{itemInfo.name}</h2>
                      <p className="text-gray-600 text-sm mb-1">{itemInfo.description || "Delicious food item"}</p>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center px-8 py-6 bg-green-100  ">
              ...
              <button className="mt-4 md:mt-0 bg-green-600 hover:scale-105 hover:bg-green-700 text-white font-semibold px-8 py-3 rounded-xl shadow-lg transition-all duration-300">
                ✅ Checkout
              </button>
            </div>
          </div>
